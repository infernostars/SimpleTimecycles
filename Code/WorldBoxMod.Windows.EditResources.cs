...
namespace PowerBox
{
    public class buttonActivation : MonoBehaviour
    {
        void Awake()
        {
            ...
            btn.onClick.AddListener(WorldBoxMod.Edit_Resouces_Button_Click);
        }
    }

    partial class WorldBoxMod : MonoBehaviour
    {
        private static void initEditResoucesWindow(Transform inspect_villageContent)
        {
            ...
            var editResouces = Helper.GodPowerTab.createButton(
                "EditResouces",
                Mod.EmbededResources.LoadSprite(resources + ".powers.res_clear.png", 0, 0),
            ...
            editResouces.GetComponent<Image>().sprite = Mod.EmbededResources.LoadSprite(resources + ".other.backgroundBackButtonRev.png", 0, 0);
        ...
        public static void Edit_Resouces_Button_Click()
        {
            ...
            initEditResources(window, editResourseButtonCallBack);
            window.clickShow();
        }

        private static void initEditResources(ScrollWindow window, Action<ButtonResource> callback)
        {
        ...
        private static void loadResourceButton(ResourceAsset asset, int quantity, int pIndex, int pTotal, ButtonResource resourceButtonPref, Transform parent, Action<ButtonResource> callback)
        {
            ...
            //nameInputElement.GetComponent<Image>().sprite = Mod.EmbededResources.LoadSprite(resources + ".other.windowNameEdit.png", 0, 0);
        ...
        public static IEnumerator InitResourceButtonEvents(ButtonResource resButton, ResourceAsset asset, int quantity, Action<ButtonResource> callback)
        {
        ...
        private static void showTooltip(ResourceAsset asset, int quantity, GameObject resourceButton)
        {
            if (!tooltipsActive)
                return;
        ...
        private static void CheckInput(NameInput text, ResourceAsset asset, CityData cityData)
        {
            int number;
